package view.utility.stat;

import model.entity.stats.SneakStatsOwnership;
import model.entity.stats.StatsOwnership;


public class RangedWeaponStat extends Stat {
	public RangedWeaponStat(String nam, StatsOwnership p){
		super(nam, p);
	}
	
	public SneakStatsOwnership getOwner(){
		return (SneakStatsOwnership) super.getOwner();
	}
	
	public void level(){
		getOwner().upRangedWeapon();
	}
	
	public int getMag(){
		return getOwner().getStats().getRangedWeapon();
	}
}
